...
Value GetNetworkHashPS(int lookup, int height) {
    ...
    if (lookup <= 0)
        lookup = pb->nHeight % Params().GetConsensus().DifficultyAdjustmentInterval() + 1; // TODO
...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or argentum.conf setting gen)\n"
...
Value generate(const Array& params, bool fHelp)
{
    ...
    {
        unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey, miningAlgo));
        ...
        while (!CheckProofOfWork(pblock->GetPoWHash(algo), pblock->nBits, Params().GetConsensus())) {
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Argentum is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Argentum is downloading blocks...");
...
Value getauxblockbip22(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty() && !Params().MineBlocksOnDemand())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
                           "Argentum is not connected!");

    if (IsInitialBlockDownload() && !Params().MineBlocksOnDemand())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                           "Argentum is downloading blocks...");
